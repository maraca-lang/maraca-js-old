// const x = [
//   [
//     [{ type: 'function', value: '=' }, { type: 'value', value: 'x' }],
//     [
//       [{ type: 'function', value: '=>' }, { type: 'value', value: 'a' }],
//       [
//         [{ type: 'value', value: 'a' }, { type: 'context' }],
...
//       ],
//       [
//         [{ type: 'function', value: '=' }, { type: 'value', value: 'b' }],
//         { type: 'value', value: 2 },
//       ],
//     ],
//   },
...
const wrap = func => (data, ...args) =>
  func(data || { number: {}, string: {}, indices: [] }, ...args);
const tableFuncs = {
  get: wrap(
    (data, key) =>
      data[typeof key][
        key === -1 ? data.indices[data.indices.length - 1] : key
      ] || null,
  ),
  set: wrap((data, key, value) => {
    if (value[0] === null) {
      const result = { ...data, [typeof key]: { ...data[typeof key] } };
      delete result[typeof key][key];
      if (
        Object.keys(result.number).length === 0 &&
        Object.keys(result.string).length === 0
      ) {
        return null;
      }
      if (typeof key === 'number') {
        const i = result.indices.indexOf(key);
    ...
    return {
      ...data,
      [typeof key]: { ...data[typeof key], [key]: value },
      ...(typeof key === 'number'
        ? {
            indices:
              data.indices.indexOf(key) === -1
                ? [...data.indices, key].sort((a, b) => a - b)
                : data.indices,
          }
        : {}),
    };
  }),
  append: wrap((data, value) => {
    if (value[0] === null) return data;
    const last = data.indices[data.indices.length - 1] || 0;
    return {
      ...data,
      number: { ...data.number, [last + 1]: value },
      indices: [...data.indices, last + 1],
    };
  }),
  onlyKeys: wrap(
    data =>
      Object.keys(data.string).length === 0
        ? null
        : { string: data.string, number: {}, indices: [] },
  ),
};
...
const flattenStream = streamStream => {
  const getNext = ([value, c1, id], c) => [value, Math.max(c, c1), id];
  return createStream(
    () => {
      const [stream, count] = streamStream();
      return getNext(stream(), count);
    },
    emit => {
      let stopInner;
      const stopOuter = streamStream(([stream, count]) => {
        if (stopInner) stopInner();
        emit(getNext(stream(), count));
        stopInner = stream(data => emit(getNext(data, count)));
      });
      return () => {
        if (stopInner) stopInner();
        stopOuter();
...
const combineStreams = (streams, func) =>
  createStream(
    () => {
      const data = streams.map(stream => stream());
      return [func(data), Math.max(...data.map(d => d[1]))];
    },
    emit => {
...
const mapStream = (stream, func) => {
  const map = data => {
    ...
    const [value, id] = Array.isArray(result) ? result : ([result] as any);
    return [value, data[1], id];
  };
  return createStream(
    () => map(stream()),
...
const createConstant = data => createStream(() => [data, 0]);

const functions = {
  '::': key$ =>
    mapStream(key$, ([key]) => (value$, table$) =>
      combineStreams([value$, table$], ([data, [table]]) =>
        tableFuncs.set(table, key, data),
      ),
    ),
  append: (value$, table$) =>
    combineStreams([value$, table$], ([data, [table]]) =>
      tableFuncs.append(table, data),
    ),
  '+': a$ =>
    mapStream(a$, ([a]) => b$ => mapStream(b$, ([b]) => (a || 0) + (b || 0))),
  '-': a$ =>
    mapStream(a$, ([a]) => b$ => mapStream(b$, ([b]) => (a || 0) - (b || 0))),
  '/': a$ =>
    mapStream(a$, ([a]) => b$ => mapStream(b$, ([b]) => (a || 0) / (b || 0))),
...
const toFunc = x => {
  if (x === null || typeof x === 'number' || typeof x === 'string') {
    return value$ =>
      mapStream(value$, ([v]) => {
        if (isObject(v)) return tableFuncs.get(v, x);
        if (x === null || typeof v === typeof x) {
          if (typeof v === 'number') return (v || 0) * (x || 0);
...
const runStatement = (config, scope) => {
  if (typeof config === 'function') return config;
  if (Array.isArray(config)) {
    const [func, value] = config;
    const funcStream = runStatement(func, scope);
    const valueStream = runStatement(value, scope);
    const { context, table } = scope;
    scope.context = flattenStream(
      mapStream(
        funcStream,
        ([func]) =>
          toFunc(func).length === 2 ? func(valueStream, context) : context,
      ),
    );
    scope.table = flattenStream(
      mapStream(
        funcStream,
        ([func]) =>
          toFunc(func).length === 2 ? func(valueStream, table) : table,
      ),
    );
    return flattenStream(
      mapStream(
        funcStream,
        ([func]) =>
          toFunc(func).length === 2
            ? createConstant(null)
            : toFunc(func)(valueStream),
      ),
    );
  }
  ...
  if (type === 'define') {
    ...
    return createConstant(stream$ =>
      run(
        [[[{ type: 'function', value: '=' }, input], stream$], output],
        context,
      ),
    );
  }
  if (type === 'table') return run(value, scope.context);
  if (type === 'function') return createConstant(functions[value]);
  if (type === 'value') return createConstant(value);
  if (type === 'context') return scope.context;
};

const run = (configArray, context) => {
  const scope = {
    context: mapStream(context, ([c]) => tableFuncs.onlyKeys(c)),
    table: createConstant(null),
  };
  configArray.forEach(config =>
    runStatement([{ type: 'function', value: 'append' }, config], scope),
  );
  return scope.table;
};

export default config => run(config, createConstant(null))()[0];
